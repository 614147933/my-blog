//参考 https://blog.csdn.net/m0_37480079/article/details/88018402?utm_medium=distribute.pc_relevant.none-task-blog-2~default~baidujs_title~default-0.pc_relevant_default&spm=1001.2101.3001.4242.1&utm_relevant_index=3
<template>
  <div class="container">
    <div class="layout">
      <button
...
export default defineComponent({
  setup() {
    let stargindex = ref('')
    let endIndex = ref('')
    let layoutType = ref(9)
    let layoutOptions = reactive([
      { label: "单分屏", value: 1 },
      { label: "二分屏", value: 2 },
      { label: "三分屏", value: 3 },
      { label: "四分屏", value: 4 },
    ...
    const onDragstart =(event:any) => {
      stargindex = event.target.getAttribute("data-id");
    }
    const onDragend = () => {
      let startGroupIndex = stargindex.split("-")[0];
      let startChildIndex = stargindex.split("-")[1];
      let endGroupIndex = endIndex.split("-")[0];
      let endChildIndex = endIndex.split("-")[1];
      // 对数据做简单的深拷贝 目前不需要
      ...
      // );
      let endObj = data[endGroupIndex].children[endChildIndex];
      let startObj = data[startGroupIndex].children[startChildIndex];
      if (data[endGroupIndex].group === "source") {
        //往资源区拖拽时  不做任何替换操作
        return;
      }
      data[endGroupIndex].children.splice(endChildIndex, 1, startObj);
      if (data[startGroupIndex].group !== "source") {
        //拖拽起始区域不是 source时  把起始区域替换成拖拽后区域的数据
        data[startGroupIndex].children.splice(startChildIndex, 1, endObj);
      }
    }
    const onDrop = (event:any) => {
      if (event.target.className.indexOf("cls-default") > -1) {
        endIndex = event.target.getAttribute("data-id");
      } else {
        endIndex = event.target.parentElement.getAttribute("data-id");
      }
    }
